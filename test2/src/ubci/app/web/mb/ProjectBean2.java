...
@ViewScoped
public class ProjectBean2 implements Serializable {
	...
	List<ProjectService> list = new ArrayList<ProjectService>();
	...
	public void init() {
		...
		for (int i = 0; i < projects.size(); i++) {
			...
			try {
				p.setDa(attributService.findType_DAById(idda).getLibélé());
			...
			try {
				p.setEntité(attributService.findEntitéById(identite)
						.getLibélé());
			...
			try {
				p.setApp(attributService.findApplicationById(idapp)
						.getLibélé());
			...
			try {
				p.setNomChefP(userService.findUserById(idChefP).getLogin());
			...
			try {
				p.setNomSponsor(userService.findUserById(idSponsor)
						.getLogin());
			...
			try {
				p.setNomMOA(userService.findUserById(idMOA).getLogin());
			...
			try {
				p.setP(attributService.findPrioritéById(idp)
						.getLibélé());
			...
			try {
				p.setS(attributService.findStatutById(ids)
						.getLibélé());
			...
			try {
				p.setR(attributService.findReliquatById(idr)
						.getLibélé());
			...
			try {
				p.setE(attributService.findEnjeuxById(ide)
						.getLibélé());
			} catch (Exception ex) {
				p.setE("");
			}
			
			p.setLibélé(projects.get(i).getLibélé());
			p.setLancerMetier(projects.get(i).getLancerMetier());
			p.setLc(projects.get(i).getLC());
			p.setId(projects.get(i).getId());
	...
	public List<ProjectService> createProjectServices(int size) {
